...
```{r}
mpsz = st_read(dsn = "K:/kalpitkulshrestha24/ISSS626/Hands-on_Exercise/Hands-on EX07/data/geospatial",
...
condo_resale = read_csv(
  "K:/kalpitkulshrestha24/ISSS626/Hands-on_Exercise/Hands-on EX07/data/aspatial/Condo_resale_2015.csv")
```
...
## selecting the sub zone area
here, i have selcted clementi to get more about this sub-zone region in the singapore.
```{r}
sg_pa_Novena <- mpsz_svy21 %>% 
  filter(PLN_AREA_N == "NOVENA")
```

```{r}
map_novena <- tm_shape(mpsz_svy21) +
  tm_polygons(col = "grey95", border.col = "black", lwd = 0.3) +
  tm_shape(sg_pa_Novena) +
  tm_polygons(col = "steelblue", border.col = "black", lwd = 1.2) +
  tm_layout(title = "Selected Planning Area: Novena",
            frame = FALSE)

```
```{r}
tmap_arrange(map_novena)
```
